...
ll convert(std::string s) {
    ll res = 0, pow = 1;
    for (int i = isz(s)-1; i >= 0; i--, pow *= 5) {
        ll curr = -3;
        if (s[i] == '=') {
            curr = -2;
        } else if (s[i] == '-') {
...
std::string deconvert(ll x) {
    ...
    ll rem = (x % 5 + 5) % 5;
    if (rem == 4) {
        return deconvert((x + 1)/5) + digit2str(-1);
    } else if (rem == 3) {
        return deconvert((x + 2)/5) + digit2str(-2);
    } else {
        assert(0 <= rem && rem <= 2);
        return deconvert((x - rem)/5) + digit2str(rem);
    }
}
int main() {
    std::string s;
    ...
    while(std::getline(std::cin, s)) {
        ...
        ll num = convert(s);
        auto dec = deconvert(num);
        assert(s == dec);
        watch(s,num,dec);
        sum += num;
    }
    watch(sum, deconvert(sum));
}
