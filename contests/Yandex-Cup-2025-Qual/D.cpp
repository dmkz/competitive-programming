...
const int debug = 0;
#endif // __DEBUG_HPP__
// [0..N) -> спрашиваем половину, знаем [0..mid) [mid, N)
// 500 -> 250 + 250 (1 запрос)
// 125 + 125 + 125 + 125 (2 запроса)
bool STRESS = false;
struct Seg {
    int leftItem, rightItem;
    vi poses;
    Seg(int L_, int R_, vi poses_)
        : leftItem(L_), rightItem(R_), poses(poses_) { }
    int len() const {
        return rightItem - leftItem + 1;
    }
    int nPos() const {
        return isz(poses);
    }
    int mid() const {
        return (leftItem+rightItem)/2;
    }
    void check() {
        assert(leftItem <= rightItem);
        assert(len() == nPos());
        for (int i = 1; i < isz(poses); i++)
            assert(poses[i-1] < poses[i]);
    }
};
Seg buildRoot(int n) {
    vi poses;
    for (int i = 1; i <= n; i++)
        poses.push_back(i);
    return Seg(1, n, poses);
}

int juryN = 10;
vi juryPerm = {2, 4, 6, 8, 10, 9, 7, 5, 3, 1};
vi askJury(const vi &a) {
    // на каких позициях нужно разместить элементы a?
    vi juryPosOf(isz(juryPerm));
    for (int i = 0; i < isz(juryPerm); i++)
        juryPosOf[juryPerm[i]-1] = i+1;
    vi res;
    for (auto it : a) 
        res.push_back(juryPosOf[it-1]);
    std::sort(all(res));
    return res;
}
vi ask(const vi &a) {
...
void answer(const vi &answ) {
    if (STRESS) {
        assert(answ == juryPerm);
    } else {
        std::string s = "RESULT";
        for (auto it : answ) {
            //std::cout << ' ' << it;
...
void solve() {
    ...
    std::vector<Seg> segs(1, buildRoot(n));
    while (true) {
        bool divided = false;
        vi question = {};
        ...
        if (divided) {
            ...
            for (const auto &s : segs) {
                if (s.poses.size() >= 2u) {
                    // L = 1, R = 2 -> mid = 1;
                    int mid = s.mid();
                    Seg left(s.leftItem, mid, vi{});
                    Seg right(mid+1, s.rightItem, vi{});
                    for (int x : s.poses)
                        if (std::binary_search(all(res), x))
                            left.poses.push_back(x);
                        else
                            right.poses.push_back(x);
                    left.check();
                    right.check();
                    next.push_back(left);
...
void stress() {
    STRESS = 1;
    for (juryN = 1; juryN <= 10; juryN++) {
        juryPerm.resize(juryN);
        for (int i = 0; i < juryN; i++)
            juryPerm[i] = i+1;
        do {
            solve();
        } while (std::next_permutation(all(juryPerm)));
        std::cout << "juryN = " << juryN << ": OK" << std::endl;
    }
    STRESS = 0;
}
main() {
