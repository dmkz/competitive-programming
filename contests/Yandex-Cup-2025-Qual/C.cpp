...
/*
    Хотим k = 10:
    1 2 3 4 5 6  7  8  9
    1 1 2 3 5 #  5  5 10  fib[5] * fib[3] = 5 * 2 = 10
    1 1 2 3 5 8 13 21 34
    1 1 2 3 5 8 13  # 13
    1 1 2 3 5 8  #  8  8  fib[6] * fib[2]
    1 1 2 3 5 #  5  5 10
    1 1 2 3 # 3  3  6  9
    1 1 2 # 2 2 4 6 10
*/
const std::vector<ll> fib = [](){
    std::vector<ll> fib = {0, 1};
    while (fib[isz(fib)-2] + fib[isz(fib)-1] <= (ll)1e18L)
        fib.push_back(fib[isz(fib)-2] + fib[isz(fib)-1]);
    return fib;
}();
std::vector<pll> stack;
ll answers{};
void f(int i, ll k, int n) {
    //watch(i, k, stack);
    if (k == 1) {
        //bool debug = 1;
        watch(i, k, stack);
        answers++;
        ll initK = 1;
        ll required = std::max(0, isz(stack)-1);
        for (auto &[len, val] : stack) {
            required += len;
            initK *= val;
        }
        if (required <= n) {
            vl answ;
            ll total = 1;
            for (auto &[len, _] : stack) {
                total += len;
                if (total < n) {
                    answ.push_back(total);
                    total++;
                }
            }
            while (total + 2 <= n) {
                answ.push_back(total+1);
                total += 2;
            }
            //watch(answ);
            std::string s(n, '.');
            ...
            if (dp.back() == initK) {
                std::cout << isz(answ) << std::endl;
                for (auto it : answ)
                    std::cout << it << ' ';
                std::cout << std::endl;
                std::exit(0);
            }
            //bool debug = (dp.back() != initK);
            //watch(s);
            //watch(dp);
            //watch(answ);
            //watch(initK);
            //watch(n);
            //assert(dp.back() == initK);            
        }
        return;
    }
    for (; i < isz(fib) && fib[i] <= k; i++) {
        //watch(k, fib[i], k % fib[i]);
        if (k % fib[i] == 0) {
            stack.emplace_back(i, fib[i]);
            f(i, k / fib[i], n);
            stack.pop_back();
        }
    }
