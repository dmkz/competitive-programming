...
const int debug = 0;
#endif // __DEBUG_HPP__
bool check(int n1, int n2, int rows, int cols) {
    //bool debug = (n1 == 4 && n2 == 5);
    // нечёт или чёт
    int nPairs = (cols / 2);
    int nFree = (cols - 2 * nPairs) * rows;
    nPairs *= rows;
    watch(n1, n2, rows, cols);
    watch(nPairs, nFree);
    if (nFree == 0) {
        if (n1 % 2 == 1 || n2 % 2 == 1)
            return false;
...
void solve() {
    ...
    int sq = n1 + n2;
    bool ok = false;
    for (int rows = 1; rows <= sq; rows++) {
        if (sq % rows == 0) {
            int cols = sq / rows;
            if (cols == rows && check(n1, n2, rows, cols)) {
                ok = true;
                break;
            }
        }
