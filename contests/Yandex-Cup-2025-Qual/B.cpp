...
int solveBarrier(const vvi &a) {
    const int nRows = isz(a);
    const int nCols = isz(a[0]);
    // прямоугольник из нулей максимальной площади
    vvi dp(nRows, vi(nCols));
    for (int i = nRows-1; i >= 0; i--)
        for (int j = 0; j < nCols; j++)
            dp[i][j] = (a[i][j] == 0) ? (1 + (i+1 < nRows ? dp[i+1][j] : 0)) : 0;
    ...
    for (int i = 0; i < nRows; i++) {
        vpii stack;
        for (int j = 0; j < nCols; j++) {
            stack.emplace_back(dp[i][j], j);
            while (isz(stack) >= 2 &&
                   stack[isz(stack)-2].first >= stack[isz(stack)-1].first)
            {
                auto &[hei, index] = stack[isz(stack)-2];
                remax(answ, (j - index) * hei);
                hei = stack[isz(stack)-1].first;
                stack.pop_back();
            }
            watch(j, stack);
        }
...
int solve(vvi a) {
    for (auto &vec : a)
        vec.push_back(1);
    return solveBarrier<debug>(a);
}
int slow(const vvi &a) {
    const int nRows = isz(a);
    const int nCols = isz(a[0]);
    vvi dp(nRows+1, vi(nCols+1));
    for (int i = nRows-1; i >= 0; i--)
        ...
        for (int i2 = i1; i2 < nRows; i2++)
            for (int j1 = 0; j1 < nCols; j1++)
                for (int j2 = j1; j2 < nCols; j2++) {
                    int s = dp[i1][j1] - dp[i2+1][j1] - dp[i1][j2+1] + dp[i2+1][j2+1];
                    if (s == 0) {
                        remax(answ, (i2-i1+1)*(j2-j1+1));
                    }
                }
    return answ;
}
...
void solve() {
    ...
    nCols++;
    std::cout << solveBarrier(a) << std::endl;
}
void stress() {
