...
int main() {
    int n; std::cin >> n;
    std::vector<ll> prev = {0,0,1};
    for (int i = 1; i <= n; i++) {
        std::vector<ll> curr(3, 0);
        for (int currItem = 0; currItem <= 2; currItem++) {
            for (int prevItem = 0; prevItem <= 2; prevItem++) {
                if (currItem == 0 && prevItem == 0) {
                    continue;
                }
                curr[currItem] += prev[prevItem];
            }
        }
        prev = curr;
    }
    std::cout << std::accumulate(all(prev), ll(0)) << '\n';
}