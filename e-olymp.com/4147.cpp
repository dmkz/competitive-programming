/*
    Задача: 4147. Двоичное дерево поиска 2
...
int main() {
    ...
    while (std::cin >> command >> value) {
        if (command == "insert") {
            ...
            int group, pos;
            bool flag = set.UpperBound(value, group, pos);
            if (flag) {
                std::cout << set.Get(group, pos) << "\n";
            } else {
                std::cout << "none\n";
            }
        } else if (command == "prev") {
            int group, pos;
            bool flag = set.LowerBound(value, group, pos);
            flag = set.Prev(group, pos);
            if (flag) {
                std::cout << set.Get(group, pos) << "\n";
            } else {
                std::cout << "none\n";
            ...
            int group, pos;
            bool flag = set.ItemByOrder(value, group, pos);
            if (flag) {
                std::cout << set.Get(group, pos) << "\n";
            } else {
                std::cout << "none\n";
