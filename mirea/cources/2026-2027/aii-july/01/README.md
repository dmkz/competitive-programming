...
Посмотрим на компактный олимпиадный вариант.
...
> [!NOTE]
> Запись параметров функции через `auto` доступна с C++20.
...
Файлы примера: [C++](Part-03-Sorting/examples/example-15.cpp), [Python3](Part-03-Sorting/examples/example-15.py).
...
для компаратора писать нельзя, иначе из определения компаратора следует, например, что `x` меньше чем `x`.
...
**Бинарный поиск** — это способ эффективного поиска элементов в **отсортированном** массиве.

Проверим, есть ли `x` в массиве. Идея:
- смотрим на элемент в середине массива;
- если он меньше `x`, то отбрасываем левую половину;
- если он больше `x`, то отбрасываем правую половину;
- продолжаем так, пока не найдём ответ.

За один шаг мы отбрасываем примерно половину вариантов, поэтому время работы алгоритма равно `O(log n)`.
...
int main() {
    ...
    while (left <= right) {
        ...
        if (a[mid] < x) {
            // элемент x в правой половине
            left = mid + 1;
        } else {
            // элемент x в левой половине
            right = mid - 1;
        }
    }
...
В спортивном программировании очень часто приходится искать **первый элемент, который больше либо равен `x`**. Именно эта версия бинарного поиска лежит в основе `lower_bound`.
...
## 26. Как проверить, что элемент действительно найден

Если нужно найти сам элемент `x`, то после вызова `lower_bound` важно не забыть проверить, действительно ли найденный итератор указывает на `x`.

> [!NOTE]
> Если подходящего элемента в массиве нет, то `lower_bound` возвращает `end()`.
> В Python3 аналогичная ситуация для `bisect_left` означает, что результат равен `len(a)`.

### C++

```cpp
auto it = lower_bound(a.begin(), a.end(), x);
if (it != a.end() && *it == x) { // разыменовывать end() нельзя!
    cout << "YES\n";
} else {
    cout << "NO\n";
}
...
Если известны результаты `lower_bound` и `upper_bound`, то можно посчитать количества меньших, равных и больших элементов.
...
Для решения этой задачи не существует готовой функции, однако искомый элемент стоит **сразу перед** `lower_bound`.
...
Если уже умеем находить первый элемент `>= x`, то бесплатно получаем и максимальный элемент `< x`.
...
Таким образом, находим первый элемент `>= x`, а затем просто переходим на одну позицию влево.
...
### 5. Берёте предыдущий элемент, когда его нет
