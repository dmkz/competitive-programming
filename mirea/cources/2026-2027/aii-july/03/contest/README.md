...
> **Предупреждение.** Данный способ содержит продвинутые техники и позволяет решить задачу на Python3 без собственной реализации дерева поиска. Он точно не для начинающих, поэтому можете пропустить его, если пока не готовы.
