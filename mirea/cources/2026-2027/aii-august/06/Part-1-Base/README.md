...
$$
\alpha=\operatorname{atan2}(\vec a\times\vec b,\vec a\cdot\vec b).
$$
...
> $$
> \alpha=\operatorname{atan2}(y_2,x_2)-\operatorname{atan2}(y_1,x_1),
> $$
...
Сама формула $\operatorname{atan2}(\vec a\times\vec b,\vec a\cdot\vec b)$ также корректна и сразу возвращает нормализованный ориентированный угол. Однако при целочисленных координатах необходимо следить, чтобы вычисление произведений не вызвало переполнение. В следующем примере показан универсальный вариант через разность полярных углов.
