...
## E. Видимость с горных вершин

<details><summary><b>Идея</b></summary>

Зафиксируем один горный пик и сначала будем искать видимые вершины только справа от него.

Пусть рассматривается вершина $r$. Отрезок от исходного пика до неё не должен пересекать склон между своими концами. Поскольку склон состоит из отрезков, достаточно проверить все промежуточные вершины: если каждая из них лежит строго ниже луча зрения, то и соединяющие их участки склона лежат ниже. Если же хотя бы одна вершина лежит на луче или выше него, видимость закрыта.

Это условие удобно выразить через угол направления. При движении слева направо новая вершина видна тогда и только тогда, когда направление на неё расположено строго выше направлений на все предыдущие вершины.

Важно искать максимальный угол, а не вершину с максимальной координатой $y$. Более высокая, но далёкая точка может образовывать меньший угол, чем более низкая, расположенная ближе.

<img src="pictures/05.svg" alt="Пятое изображение">

На рисунке $H$ выше $M$, однако направление $OM$ имеет больший угол. Более того, склон пересекает отрезок $OH$, поэтому $H$ не видна. Следовательно, поддерживать только самую высокую вершину недостаточно.

<img src="pictures/05-1.svg" alt="Ещё одна иллюстрация">

Осталось выяснить, среди каких точек нужно поддерживать максимальный угол. Ответ — среди **всех вершин ломаной**, а не только среди горных пиков. Обычная вершина тоже является частью склона и может пересечь луч зрения. Если игнорировать её лишь потому, что она не является пиком, можно ошибочно объявить следующий пик видимым.

Итак, при проходе от выбранного пика вправо мы поддерживаем вершину, задающую максимальное направление среди всех уже просмотренных вершин. Очередная вершина видна, только если она устанавливает новый строгий максимум. При этом увеличивать ответ нужно лишь тогда, когда сама новая видимая вершина является горным пиком.

Для левой стороны применяется такое же рассуждение с противоположным порядком углов.

</details>

<details><summary><b>Сравнение направлений</b></summary>

Делить разности координат и вычислять углы через вещественные числа не требуется. Два направления можно сравнить с помощью векторного произведения.

Пусть исходный пик — точка $p_i$, текущий максимум задаёт точка $p_m$, а проверяемая вершина — $p_r$. Рассмотрим векторы

$a=p_m-p_i$,

$b=p_r-p_i$.

Для правой стороны их координаты $x$ положительны. Если

$\text{cross}(a,b)>0$,

то вектор $b$ расположен выше вектора $a$, то есть текущая вершина устанавливает новый максимальный угол.
...
Неравенство обязательно должно быть строгим. Если векторное произведение равно нулю, текущая вершина лежит на уже достигнутом луче зрения. Тогда соединяющий пики отрезок касается склона в промежуточной точке, а по условию даже касание закрывает обзор.

При проходе влево абсциссы обоих векторов отрицательны, поэтому порядок знака меняется: новый верхний луч определяется условием $\text{cross}(a,b)<0$.

<img src="pictures/05-2.svg" alt="Ещё одна иллюстрация">

</details>

<details><summary><b>Поиск горных пиков</b></summary>

Сначала отдельно отметим вершины, для которых вообще нужно вывести ответ. Вершина является горным пиком, если она внутренняя и расположена строго выше обеих соседних вершин.
...
Крайние вершины не могут быть пиками по определению, поскольку у каждой из них нет двух соседей.

</details>

<details><summary><b>Проход вправо и влево</b></summary>

Зафиксируем пик $i$. Его соседняя справа вершина $i+1$ становится первым ограничивающим направлением. Она не может быть пиком, поскольку находится ниже $i$.

Затем вершины перебираются слева направо. Если очередное направление строго выше текущего максимума, вершина видна и становится новым максимумом. В ответ она добавляется только в том случае, если является пиком.

При этом максимум обновляется независимо от того, является ли вершина пиком. Именно это учитывает возможность того, что обзор закроет обычная вершина склона.
...
Левая сторона обрабатывается симметрично. Первым ограничивающим направлением становится вершина $i-1$, после чего индексы перебираются по убыванию.
...
Во втором примере пики имеют номера $2$, $4$ и $6$.

Из пика $2$ виден пик $4$. Пик $6$ расположен дальше, но направление на него не устанавливает новый строгий максимум: склон между ними достигает луча зрения и закрывает обзор. Поэтому ответ для пика $2$ равен $1$.

Из среднего пика $4$ видны оба соседних пика — один слева и один справа. Для него ответ равен $2$.

По симметричному рассуждению из пика $6$ виден только пик $4$. Поэтому выводятся строки:
...
Сначала выводится количество найденных горных пиков. Затем вершины просматриваются в порядке их нумерации, и для каждого пика выводятся его номер и найденное количество видимых пиков.
...
Докажем корректность алгоритма для одного фиксированного горного пика $i$.

**Лемма 1.** Пик $r>i$ виден из $i$ тогда и только тогда, когда направление из $i$ в $r$ строго выше направлений из $i$ во все вершины с индексами от $i+1$ до $r-1$.

Если направление на некоторую промежуточную вершину не ниже направления на $r$, эта вершина лежит на отрезке зрения или выше него. В первом случае происходит запрещённое касание, во втором склон пересекает отрезок зрения. Поэтому пик $r$ не виден.

Если же все промежуточные вершины строго ниже отрезка $p_ip_r$, то каждый участок ломаной между двумя соседними промежуточными вершинами также лежит строго ниже этого отрезка. Следовательно, склон не имеет с отрезком зрения общих точек, кроме концов, и пик $r$ виден.

**Лемма 2.** Во время прохода вправо точка $m$ задаёт максимальное направление среди всех уже рассмотренных вершин.

Сначала рассмотрена только вершина $i+1$, поэтому утверждение выполняется. Затем для каждой новой вершины её направление сравнивается с направлением на $m$. Если оно строго выше, максимум обновляется; иначе прежний максимум сохраняется. Следовательно, инвариант выполняется после каждого шага.

**Лемма 3.** Алгоритм правильно определяет все пики, видимые справа от $i$.

По лемме 2 перед обработкой вершины $r$ точка $m$ задаёт максимум среди всех промежуточных вершин. Условие `cross(bestVec, currVec) > 0` выполняется ровно тогда, когда направление на $r$ строго выше этого максимума. По лемме 1 это равносильно видимости вершины $r$.

Если вершина видима и является пиком, алгоритм увеличивает ответ. Если она не является пиком, ответ не меняется, но максимум всё равно обновляется, поскольку такая вершина способна закрыть дальнейшие пики.

**Лемма 4.** Алгоритм правильно определяет все пики, видимые слева от $i$.

При движении справа налево действует тот же критерий видимости. Из-за отрицательных абсцисс направляющих векторов новый верхний луч соответствует условию `cross(bestVec, currVec) < 0`. Поэтому левый проход учитывает каждый и только каждый видимый пик слева.

**Теорема.** Для каждого горного пика алгоритм выводит число всех других пиков, видимых из него.

По лемме 3 правильно подсчитываются все видимые пики справа, а по лемме 4 — все видимые пики слева. Эти множества не пересекаются и вместе содержат все остальные пики. Следовательно, их суммарное количество является требуемым ответом.

</details>

<details><summary><b>Сложность</b></summary>

Для каждого горного пика выполняются два линейных прохода по вершинам. Пиков не больше $n$, поэтому время работы одного набора данных составляет $O(n^2)$.

Сумма $n$ по всем наборам не превосходит $2000$. Сумма квадратов не превосходит квадрата суммы, поэтому суммарное число операций укладывается в $O(2000^2)$.

Массивы координат, признаков пиков и ответов занимают $O(n)$ дополнительной памяти.

Векторное произведение может достигать величины порядка $10^{12}$, поэтому в C++ для него используется `int64_t`.
