...
> \alpha_{\text{рад}}
> =
> \alpha_{\text{град}}\cdot\frac{\pi}{180}
> $$
...
$$
\left|\vec{PA}\times\vec{PB}\right|
+
\left|\vec{PB}\times\vec{PC}\right|
+
\left|\vec{PC}\times\vec{PA}\right|
=
\left|\vec{AB}\times\vec{AC}\right|
$$
...
$$
s_1=\operatorname{cross}(A,B,P),\qquad
s_2=\operatorname{cross}(B,C,P),\qquad
s_3=\operatorname{cross}(C,A,P)
$$
...
S_{\cap}
=
\max\left(0,x_R-x_L\right)
\cdot
\max\left(0,y_T-y_B\right)
}
$$
...
$$
w=\max\left(0,\min(x_{\max}^{(1)},x_{\max}^{(2)})
-\max(x_{\min}^{(1)},x_{\min}^{(2)})\right)
$$

$$
h=\max\left(0,\min(y_{\max}^{(1)},y_{\max}^{(2)})
-\max(y_{\min}^{(1)},y_{\min}^{(2)})\right)
$$
