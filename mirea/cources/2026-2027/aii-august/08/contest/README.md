...
<p align="center">
  <img src="pictures/c-one-cell-per-diagonal.svg" alt="Маршрут пересекает диагональ с ловушками только один раз" width="900">
</p>
<p align="center">
  <em>Рисунок 1. При каждом ходе сумма координат r + c увеличивается на единицу, поэтому маршрут посещает ровно одну клетку каждой диагонали. Если все ловушки лежат на одной диагонали, маршрут не может пройти через две из них.</em>
</p>
...
<p align="center">
  <img src="pictures/c-path-through-trap.svg" alt="Разбиение маршрута в клетке с ловушкой на две части" width="900">
</p>
<p align="center">
  <em>Рисунок 2. Маршрут через ловушку X однозначно состоит из пути от начальной клетки S до X и пути от X до конечной клетки F. Поэтому количества вариантов для двух частей перемножаются.</em>
</p>
...
## E. Треугольник поглощает плоскость

<details><summary><b>Основная идея</b></summary>

Через $t$ секунд каждая вершина сдвигается на расстояние $t$ вдоль луча, выходящего из начала координат. При увеличении времени треугольник **только расширяется**: если все точки поглощены в момент $t$, то они останутся внутри и в любой более поздний момент. Это даёт монотонное условие для бинарного поиска по ответу.

Для проверки выбранного времени нужно построить новое положение трёх вершин и проверить принадлежность всех данных точек полученному треугольнику.

В предоставленных реализациях используются разные способы проверки:

- в C++ благодаря типу `long double` сравниваются площади;
- в Python сравниваются ориентации точки относительно трёх сторон.

Для C++ при сравнении площадей **обязательно нужна отдельная погрешность `EPS`**. Точность бинарного поиска здесь не помогает: даже для точки строго внутри треугольника два математически равных выражения для площади могут немного отличаться из-за вычислений с вещественными числами.

Если заранее неизвестна правая граница бинарного поиска, начнём с небольшого значения и будем удваивать его, пока треугольник не поглотит все точки.

Одна проверка работает за $O(n)$. Общее время составляет $O(n\log(R/\delta))$, где $R$ — найденная верхняя граница ответа, а $\delta$ — требуемая точность поиска. Память — $O(n)$.

</details>

<details><summary><b>Детальный разбор</b></summary>

Положение каждой вершины меняется непрерывно, причём вершина движется вдоль луча из начала координат с единичной скоростью. Для вершины $V$ её положение в момент $t$ равно

$V(t)=V+t\cdot\dfrac{V}{|V|}$.

Вектор $V/|V|$ имеет длину $1$, поэтому добавление этого вектора, умноженного на $t$, действительно увеличивает расстояние от начала координат ровно на $t$.

Непосредственно вычислить момент поглощения каждой точки сложно: стороны треугольника одновременно меняют и положение, и наклон. Однако нам не обязательно находить ответ по явной формуле. Достаточно уметь отвечать на вопрос:

**поглощены ли все точки через заданное время $t$?**

Сначала убедимся, что ответ на этот вопрос обладает монотонностью.

Рассмотрим два момента $t_1<t_2$. Каждая вершина в момент $t_1$ лежит на отрезке между началом координат и своим положением в момент $t_2$. Начало координат находится внутри треугольника в любой момент, поэтому все три более ранние вершины лежат внутри более позднего треугольника. Вместе с ними внутри него находится и их выпуклая оболочка, то есть весь треугольник в момент $t_1$.

Следовательно, треугольники являются вложенными:

- если точка поглощена в момент $t$, она останется поглощённой позже;
- если в момент $t$ поглощены все точки, это верно для любого большего времени.

Именно это свойство позволяет применить **бинарный поиск по минимальному подходящему времени**.

<p align="center">
  <img src="pictures/e-nested-triangles.svg" alt="Вложенные положения расширяющегося треугольника" width="900">
</p>
<p align="center">
  <em>Рисунок 1. Вершины движутся вдоль лучей из начала координат. Более позднее положение треугольника содержит более раннее, поэтому условие «все точки поглощены» монотонно по времени.</em>
</p>

Чтобы проверить время $t$, для каждой исходной вершины заранее найдём единичный вектор её движения. Например, для вершины $A$ он равен

$v_A=\dfrac{A}{|A|}$.

Тогда новое положение вершины вычисляется как

$A(t)=A+v_A\cdot t$.

Такие же формулы используются для $B(t)$ и $C(t)$. После построения треугольника остаётся проверить, находятся ли внутри него все данные точки.

В реализации на C++ используется **метод площадей**. Пусть нужно проверить точку $P$ относительно треугольника $ABC$. Если $P$ находится внутри треугольника или на его границе, то треугольник разбивается этой точкой на три части, поэтому выполняется равенство

$S(PAB)+S(PAC)+S(PBC)=S(ABC)$.

В коде вычисляется удвоенная площадь. Для трёх точек $X$, $Y$, $Z$ она равна модулю векторного произведения:

$S_2(XYZ)=|(Y-X)\times(Z-X)|$.

Множитель $2$ не влияет на сравнение, зато позволяет не выполнять лишнее деление.

<p align="center">
  <img src="pictures/e-area-method.svg" alt="Разбиение треугольника внутренней точкой на три части" width="900">
</p>
<p align="center">
  <em>Рисунок 2. Если точка P находится внутри треугольника ABC или на его границе, сумма площадей треугольников PAB, PAC и PBC равна площади исходного треугольника.</em>
</p>

Здесь возникает **важная особенность вычислений с вещественными числами**. Математически площади равны точно, но после нескольких операций значения могут отличаться из-за ошибок округления. Поэтому обычная проверка на точное равенство непригодна.

Используется сравнение по абсолютной или относительной погрешности:

$|a-b|\le \varepsilon\cdot\max(1,|a|,|b|)$.

Относительная часть особенно важна для больших координат и больших площадей: допустимая ошибка должна учитывать масштаб сравниваемых величин. Абсолютная часть, задаваемая единицей внутри максимума, не позволяет требовать чрезмерной точности около нуля.

**Погрешность при сравнении площадей не связана с точностью бинарного поиска.** Это две разные задачи:

- `EPS = 1e-18` используется, чтобы корректно сравнивать площади при одной проверке;
- значение `1e-7` используется как условие завершения бинарного поиска.

Может показаться, что достаточно немного увеличить найденное время и точка окажется строго внутри. Но для метода площадей равенство суммы площадей должно выполняться не только на границе, а **для любой внутренней точки**. Без `EPS` накопленная ошибка остаётся, и математически равные площади могут не совпасть в представлении `long double`. Поэтому отсутствие `EPS` способно нарушить всю проверку независимо от количества итераций бинарного поиска.

<p align="center">
  <img src="pictures/e-eps-comparison.svg" alt="Различие между погрешностью сравнения площадей и точностью бинарного поиска" width="900">
</p>
<p align="center">
  <em>Рисунок 4. EPS при сравнении площадей компенсирует ошибки вещественных вычислений внутри check(t). Точность бинарного поиска отвечает только за близость его границ и не может заменить этот допуск.</em>
</p>

В Python применяется другой способ — **проверка ориентации векторов**. Вершины $A$, $B$, $C$ перечислены против часовой стрелки, и этот порядок сохраняется во время расширения. Точка $P$ находится внутри треугольника или на его границе тогда и только тогда, когда она лежит слева от каждой ориентированной стороны:
...
<p align="center">
  <img src="pictures/e-orientation-method.svg" alt="Проверка принадлежности точки треугольнику по ориентациям" width="900">
</p>
<p align="center">
  <em>Рисунок 3. При обходе вершин против часовой стрелки внутренняя точка P находится слева от каждой ориентированной стороны. Для внешней точки Q хотя бы одна из трёх проверок не выполняется.</em>
</p>

В этой проверке не требуется устанавливать равенство двух независимо вычисленных сумм. Для точки строго внутри все три произведения имеют положительный запас, а для внешней точки хотя бы одно из них отрицательно. Если из-за округления граница слегка сместится, бинарный поиск может выбрать совсем немного большее время, после чего точка окажется внутри с положительным запасом. Это смещение значительно меньше допустимой погрешности ответа. Поэтому в предоставленном решении проверка ориентаций работает без отдельного `EPS`.

Теперь осталось определить границы бинарного поиска. Нижняя граница изначально равна нулю. Подходящая верхняя граница заранее неизвестна, поэтому берётся небольшое положительное значение. Пока все точки ещё не поглощены, граница удваивается:

$10^{-7},\ 2\cdot10^{-7},\ 4\cdot10^{-7},\ \ldots$

Так как треугольник неограниченно расширяется во всех направлениях, через некоторое время он обязательно поглотит любой конечный набор точек. Следовательно, подходящая верхняя граница будет найдена.

После этого поддерживается следующий **инвариант**:

- `low` — время, которого ещё недостаточно;
- `high` — время, которого уже достаточно.

Для середины отрезка выполняется проверка. Если все точки уже поглощены, правую границу можно уменьшить. Иначе нужно увеличить левую границу. По завершении поиска границы отличаются не более чем на допустимую абсолютную или относительную величину, поэтому их среднее можно вывести как ответ.

Отдельно стоит учесть случай, когда все точки находятся внутри исходного треугольника. Начальная правая граница тоже проходит проверку, а бинарный поиск постепенно приближает обе границы к нулю. В результате выводится значение, достаточно близкое к нулю.

**Корректность алгоритма** следует из трёх утверждений:

1. Проверка принадлежности определяет, поглощены ли все точки в заданный момент.
2. При увеличении времени треугольник содержит все свои предыдущие положения, поэтому условие проверки монотонно.
3. Бинарный поиск сохраняет недостаточную границу слева и достаточную справа, а затем сближает их до требуемой точности.

Значит, полученное значение является минимальным временем, через которое треугольник поглотит все заданные точки, с допустимой погрешностью.

Построение единичных векторов занимает $O(1)$ времени. Каждая проверка просматривает все точки за $O(n)$. Число проверок составляет $O(\log(R/\delta))$, поэтому общая временная сложность равна $O(n\log(R/\delta))$. Для хранения координат требуется $O(n)$ памяти.

</details>

<details><summary><b>Код на C++</b></summary>

**Решение через метод площадей:**
...
**Решение через ориентацию векторов:**
...
Тип `long double` используется для более точного вычисления координат, длин и площадей.

Функция сочетает **абсолютное и относительное сравнение**. Если числа малы, масштабом служит $1$; если велики — наибольший из их модулей. Благодаря этому одна и та же функция подходит и для площадей разного масштаба, и для проверки близости границ бинарного поиска.

В коде у неё есть два разных применения:

- значение по умолчанию `EPS = 1e-18L` применяется при сравнении площадей;
- значение `1e-7` передаётся явно при сравнении границ поиска.
...
Именно она нужна для получения единичного вектора движения. Аргумент функции `sqrt` заключён в круглые скобки, как и требуется для записи математической функции.
...
Векторы `va`, `vb`, `vc` имеют длину $1$ и направлены от начала координат к соответствующим вершинам. Они не зависят от времени, поэтому вычисляются только один раз.

Поскольку начало координат строго внутри исходного треугольника, ни одна вершина не совпадает с ним. Следовательно, длины `pa`, `pb`, `pc` положительны и деление корректно.
...
Функция возвращает **удвоенную площадь** треугольника. Модуль нужен, потому что знак векторного произведения зависит от порядка обхода точек, а площадь всегда неотрицательна.

Делить результат на $2$ не требуется: в дальнейшем сравниваются суммы площадей, и общий множитель сократился бы с обеих сторон.
...
Если точка находится внутри треугольника или на границе, сумма площадей трёх частей равна площади исходного треугольника.

Здесь нельзя писать `s1 == s2`. Обе величины вычисляются разными последовательностями вещественных операций и могут немного различаться даже при точном математическом равенстве. Поэтому используется `isAlmostEqual` с малым относительным или абсолютным допуском.
...
Лямбда-функция строит положение треугольника через `t` секунд и проверяет все точки. Захват `[&]` позволяет использовать координаты, исходные вершины и единичные векторы из `solve()`.

Как только найдена первая внешняя точка, функция сразу возвращает `false`. Если цикл завершился, все точки уже поглощены.
...
Подходящая верхняя граница заранее неизвестна. Поэтому `high` удваивается до тех пор, пока проверка не станет истинной.

После завершения цикла поддерживается нужный для бинарного поиска инвариант: `low` недостаточно, а `high` уже достаточно. Если исходного треугольника достаточно, начальное значение `low = 0` может само быть подходящим, но это не мешает поиску приблизиться к нулю с требуемой точностью.
...
Монотонность проверки позволяет отбрасывать половину текущего промежутка. Если `mid` уже подходит, минимальный ответ не больше него. В противном случае ответ строго больше `mid`.

Здесь явно передано `1e-7`. Этот допуск определяет точность бинарного поиска и **не заменяет** более строгий `EPS`, который применяется внутри сравнения площадей.

**Особенность решения через ориентацию векторов**
...
В реализации через ориентации достаточно типа `double`. Здесь не сравниваются две независимо вычисленные суммы площадей, которые должны совпасть с очень высокой точностью. Вместо этого определяется знак каждого векторного произведения.
...
Вершины треугольника идут против часовой стрелки, поэтому его внутренняя область находится слева от каждой ориентированной стороны. Точка лежит внутри треугольника или на его границе, если все три векторных произведения неотрицательны.

В этой проверке отдельный `EPS` не используется. Для точки строго внутри значения имеют положительный запас. Небольшая погрешность около границы может лишь незначительно сдвинуть момент, когда `check(t)` начнёт возвращать `true`; этот сдвиг укладывается в допустимую погрешность ответа.
...
Функция проверяет близость чисел с учётом их масштаба. В этой реализации она применяется для завершения бинарного поиска с явно переданным значением `1e-7`.

Хотя константа `EPS` объявлена, проверка принадлежности треугольнику использует ориентации и не вызывает `isAlmostEqual`.
...
Длина вектора вычисляется как квадратный корень из его скалярного произведения с самим собой:

$|a|=\sqrt{a\cdot a}$.

Полученная длина используется для нормализации направления движения вершины.
...
После деления радиус-вектора каждой вершины на его длину получается единичный вектор, направленный вдоль нужного луча. Умножение такого вектора на время даёт смещение вершины ровно на это расстояние.
...
Векторное произведение двух двумерных векторов используется для определения ориентации:

- положительное значение означает левый поворот;
- отрицательное — правый;
- ноль соответствует одной прямой.
...
Вершины треугольника перечислены против часовой стрелки. Поэтому внутренняя область находится слева от каждой ориентированной стороны. Точка принадлежит треугольнику вместе с границей, если все три векторных произведения неотрицательны.

Здесь не сравниваются две величины, которые математически должны быть равны. Для внутренней точки знаки имеют положительный запас. Возможная небольшая ошибка около границы лишь незначительно сдвигает момент смены результата проверки, а бинарный поиск компенсирует это в пределах допустимой погрешности ответа.
...
Для заданного времени вычисляются новые положения вершин, после чего проверяются все точки. При обнаружении внешней точки функция немедленно завершает работу.

Результат `check(t)` монотонен: после первого подходящего времени все большие значения тоже подходят.
...
Цикл находит достаточную правую границу. После каждого неудачного вызова прежнее значение `high` становится новой недостаточной левой границей, а правая граница удваивается.
...
Если время `mid` подходит, поиск продолжается в левой половине промежутка. Если нет — в правой. Цикл завершается, когда границы становятся достаточно близки по абсолютной или относительной погрешности.

```python
print(f"{(high + low) / 2:.12f}")
```

Выводится середина оставшегося промежутка с двенадцатью знаками после десятичной точки. Этого достаточно для требуемой погрешности $10^{-6}$.
