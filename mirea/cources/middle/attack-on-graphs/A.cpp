...
main() {
    ...
    for (int x = 1; x <= 1000; x++) {
        for (int i = 1; i < isz(idByX[x]); i++) {
            int u = idByX[x][i-1], v = idByX[x][i];
            adj[u] << v;
            adj[v] << u;
        }
        for (int i = 1; i < isz(idByY[x]); i++) {
            int u = idByY[x][i-1], v = idByY[x][i];
            adj[u] << v;
            adj[v] << u;
        }
    }
