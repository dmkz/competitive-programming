...
const int inf = 1e9+7;
std::string infOrVal(int val) {
    return val >= inf ? "-1" : std::to_string(val);
}
main() {
    int nRows, nCols; std::cin >> nRows >> nCols;
    vs a(1+nRows+1, std::string(1+nCols+1, '#'));
    ...
    // dist[надет ли ранец:0 или 1][направление движения][столбец][строка]
    vector dist(2, vector(4, vector(1+nRows+1, vector(1+nCols+1, inf))));
    std::deque<t5i> q{{0,UP,startRow,startCol,0}};
    while (isz(q)) {
        auto [rocket, dir, r, c, d] = --q;
        if (dist[rocket][dir][r][c] > d) {
            dist[rocket][dir][r][c] = d;
        } else {
            continue;
        }
        // идём прямо
        {
            ...
            int nc = c + dc[dir];
            int cost = (rocket ? 0 : 1);
            int nextRocket = rocket || (a[nr][nc] == 'R');
            if (a[nr][nc] != '#') {
                if (cost == 1) q << t5i(nextRocket,dir,nr,nc,d+cost);
                else t5i(nextRocket,dir,nr,nc,d+cost) >> q;
            }
        }
        const int leftDir = (dir-1+4)%4;
        ...
        // поворот налево
        {
            ...
            int nc = c;
            int cost = (rocket ? 1 : 0);
            int nextRocket = rocket;
            if (cost == 1) q << t5i(nextRocket,leftDir ,nr,nc,d+cost);
            else t5i(nextRocket,leftDir,nr,nc,d+cost) >> q;
        }
        // поворот направо
        {
            int nr = r;
            ...
            int cost = (rocket ? 1 : 0);
            int nextRocket = rocket;
            if (cost == 1) q << t5i(nextRocket,rightDir,nr,nc,d+cost);
            else t5i(nextRocket,rightDir,nr,nc,d+cost) >> q;
        }
    }
    int answ = inf;
    for (int rocket = 0; rocket <= 1; rocket++)
        for (int dir = 0; dir < 4; dir++)
            remin(answ, dist[rocket][dir][finishRow][finishCol]);
    std::cout << infOrVal(answ) << std::endl;
}
