...
std::string infOrValue(int x) {
    if (x >= inf) return "-1";
    return std::to_string(x);
}
using vvtiii = std::vector<vtiii>;
main() {
    int n, m, k; std::cin >> n >> m >> k;
    vi c(k); std::cin >> c;
    algos::dsu::DSU dsu(n+1);
    vi vertGroup(n+1);
    vi groupBegin(k), groupEnd(k);
    for (int g = 0, p = 1; g < k; g++) {
        groupBegin[g] = p;
        for (int cnt = c[g]; cnt --> 0; vertGroup[p++] = g);
        groupEnd[g] = p;
    }
    vvi d(k, vi(k, inf));
    for (int i = 0; i < m; i++) {
        ...
        if (w == 0)
            dsu.connect(u, v);
        int iU = vertGroup[u], iV = vertGroup[v];
        if (iU != iV) {
            remin(d[iU][iV], w);
            remin(d[iV][iU], w);
        }
    }
    // для каждой группы собираем представителей всех вершин
    // если они совпали, тогда всё зашибись
    vi parForGroup(k, -1);
    bool ok = 1;
    for (int u = 1; u <= n; u++) {
        int p = dsu.getPar(u);
    ...
    for (int w = 0; w < k; w++)
        for (int u = 0; u < k; u++)
            for (int v = 0; v < k; v++)
                remin(d[u][v], d[u][w] + d[w][v]);
    std::cout << "Yes\n";
    for (int u = 0; u < k; u++) {
        for (int v = 0; v < k; v++) {
            std::cout << infOrValue(d[u][v]) << " ";
        }
        std::cout << std::endl;
    }
