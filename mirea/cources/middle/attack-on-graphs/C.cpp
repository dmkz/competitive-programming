...
main() {
    ...
    vvi a;
    {
        ...
        for (auto &s : input)
            remax(maxLen, isz(s));
        a.assign(n, vi(maxLen, 0));
        for (int i = 0; i < n; i++)
            for (int j = 0; j < isz(input[i]); j++)
                a[i][j] = int(input[i][j] - 'a' + 1);
    }
    vvi next(27), prev(27);
    std::function<void(int,int,int)> dfs = [&](int l, int r, int p) {
        if (p >= maxLen)
            return;
        vi order;
        for (int i = l, j; i < r; i = j) {
            for (j = i; j < r && a[j][p] == a[i][p]; j++);
            if (j - i >= 2)
                dfs(i, j, p+1);
            order << a[i][p];
        }
        for (int i = 1; i < isz(order); i++) {
    ...
    for (int i = 0; i < isz(order); i++)
        if (order[i] >= 1) {
            mapping[order[i]-1+'a'] = letter++;
            //std::cout << char(order[i]+'a'-1);
        }
    for (auto &s : input)
        for (auto &it : s)
            it = mapping[it];
    if (!std::is_sorted(all(input))) {
        std::cout << "Impossible" << std::endl;
        return 0;
    }
    for (int i = 0; i < isz(order); i++)
