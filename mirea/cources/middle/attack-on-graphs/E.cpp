...
main() {
    ...
    for (int k : queries) {
        ...
        for (int u : added)
            for (int v : added)
                update(d[u][v], d[u][k] + d[k][v]);
        for (int v : added)
            for (int u : added)
                update(d[k][u], d[k][v] + d[v][u]);
        for (int v : added)
            for (int u : added)
                update(d[u][k], d[u][v] + d[v][k]);
        ...
        /*for (int w : added)
            for (int u : added)
                for (int v : added)
                    remin(d2[u][v], d2[u][w] + d2[w][v]);
        std::cout << "after adding of vertex " << k << ":\n";
        std::cout << "d1:\n";
        for (int from = 1; from <= n; from++) {
            for (int to = 1; to <= n; to++) {
                std::cout << infOrValue(d[from][to]) << ' ';
            }
            std::cout << std::endl;
        }
        ...
        for (int from = 1; from <= n; from++) {
            for (int to = 1; to <= n; to++) {
                std::cout << infOrValue(d2[from][to]) << ' ';
            }
            std::cout << std::endl;
        }*/
        answ << sum;
