/*
    Problem: 1299C. Water Balance
    Solution: geometry, greedy, convex hull, O(n)
*/
#include <bits/stdc++.h>
namespace FastIO {
    struct Reader {
...
auto cross(Pt a, Pt b) { return a.x * b.y - a.y * b.x; }
int main() {
    FastIO::Reader cin;
    FastIO::Writer cout;
    cout.setprecision(9);
    int n; cin >> n;
    std::vector<Pt> st{Pt{0,0}};
    ll sum = 0;
    for (int i = 1, a; i <= n; i++) {
        cin >> a;
        Pt C{i, sum += a};
        while (isz(st) > 1) {
            Pt A = st[isz(st)-2], B = st[isz(st)-1];
            if (cross(A-B,C-B) >= 0) { st.pop_back(); }
            else break;
        }
        st.push_back(C);
    }
    for (int i = 1; i < isz(st); i++) {
        ld val = ld(st[i].y - st[i-1].y) / (st[i].x - st[i-1].x);
        for (int j = (int)st[i-1].x+1; j <= st[i].x; j++) {
            cout << val << '\n';
        }
    }
    return 0;
