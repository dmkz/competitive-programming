// двумерное динамическое программирование, O(N^2)
#include <bits/stdc++.h>
using namespace std;
int main() {
    ...
    for (int row = 0; row < n; row++) {
        for (int col = 0; col < n; col++) {
            if (a[row][col] == '1') {
                // кол-во единиц вверх:
                int U = 1 + (row-1 >= 0 ? dp[row-1][col] : 0);
                // кол-во единиц влево:
                int L = 1 + (col-1 >= 0 ? dp[row][col-1] : 0);
                ...
                int S = 1 + (row-1>=0&&col-1>=0?dp[row-1][col-1]:0);
                int res = (dp[row][col] = min({U,L,S}));
                if (answ < res * res)
                    answ = res * res;
            }
        }
