/*
    Сравнение двух подстрок за O(log(n)) при помощи двойного полиномиального хэша + бинпоиска: 
    - первый модуль простое число порядка 1e9
    - второй модуль 2^64

    Сдвиг представляем как целое число - начало подстроки.
    
    Итого, решение имеет асимптотику O(n*log(n)^2)
*/
...
int main() {
    ...
    // Сортировка сдвигов за O(n*log(n)^2), используя хэши:
    std::stable_sort(shifts.begin(), shifts.end(), [&](const int p1, const int p2) {
        ...
        assert(low < n && s[p1+low] == s[p2+low]);
        return low+1 < n && s[p1+low+1] < s[p2+low+1];
    });
